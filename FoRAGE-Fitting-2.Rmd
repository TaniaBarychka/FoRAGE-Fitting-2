...
ggplot(forage_db, aes(L.PreyBM,L.Fittted.h..day.))+geom_point(aes(col=forage_db$Habitat))+ theme(panel.border = element_blank(),panel.grid.major = element_blank(),
...
forage_db$fPreyType<-factor(forage_db$Major.grouping.1.1)

ht3<-lm(L.Fittted.h..day.~fPreyType*L.PreyBM, data=forage_db)
drop1(ht3, test="F")
...
plot(E~forage_db$L.PreyBM)
plot(E~forage_db$fPreyType)
...
```
The relationhips with predator bodymass is even less clear. Like with prey body mass there appears to be a band of animals that are realtivel big in size but have very small handling times. On the other hand we have some animals that are relatively small but have quick handling times.

I'm going to look at values for handling time of ln(ht)<=-20 abd check if they perhaps belong to a group.
